...
একটি আধুনিক, responsive Facebook Status ওয়েব অ্যাপ — Next.js (App Router) + Tailwind CSS দিয়ে তৈরি। **৬৮টি ভাষায়** emoji-সহ **১০৫০+** স্ট্যাটাস খুঁজুন, কপি করুন ও পছন্দ (favorite) করে রাখুন। দেশ অনুযায়ী ভাষা স্বয়ংক্রিয়ভাবে সনাক্ত হয়।

A modern, responsive status web app built with Next.js (App Router) and Tailwind CSS. Browse **1050+** emoji-rich statuses in **72 languages** — search, copy and favorite them with one tap. The language is auto-detected from the user's country.

## Features

- **1050+ original statuses** (English + Bangla, hand-written) auto-translated into **66 more languages** (Hindi, Urdu, Arabic, Spanish, French, German, Chinese, Japanese, Russian…)
- 7 categories: Love / ভালোবাসা, Friendship / বন্ধুত্ব, Sad / কষ্ট, Motivation / অনুপ্রেরণা, Islamic / ইসলামিক, Funny / মজার, Life / জীবন — all UI text translated too
...
- **Load More** button (24 cards per batch) for smooth performance with 1000+ statuses
- Navbar with categories dropdown + mobile menu; Footer with quick links and developer credit
- Pages: Home, Category pages, **About**, **Contact**
...
├── components/
│   ├── Navbar.jsx                 # Logo, links, categories dropdown, mobile menu, theme toggle
...
├── data/
│   ├── statuses.js                # AUTO-GENERATED: 1050 statuses EN+BN (do not edit by hand)
...
├── hooks/
│   └── usePreferences.js          # Shared hook: language, theme, favorites, toast, copy
├── lib/
│   ├── i18n.js                    # EN/BN translations + merge of generated languages
│   ├── languages.js               # 68 supported languages (code, names, flags)
...
The generator uses a fixed seed, so output is reproducible. Each category targets 150 unique bilingual statuses (1050 total).
...
Translates every unique English status text + UI string into 66 machine-translated languages (Bangla is hand-written). The script is resumable — languages with a complete cache are skipped. English and Bangla always remain the fallback.
...
- **Statuses in 72 languages**: English and Bangla texts are embedded in the status data. The other 66 languages are stored as `EN string → translation` maps in `data/translations/` and lazy-loaded only when that language is active, so the main bundle stays small.
- **Everything is translated**: UI text (buttons, tabs, placeholders, About/Contact pages, category names) comes from `lib/i18n.js` + `data/i18n-langs.js`.
